...
export default function Checkbox({ checked, onToggle }: { checked: boolean; onToggle: () => void }) {
  return (
    <button
      onClick={onToggle}
      className={`w-6 h-6 flex items-center justify-center rounded-full border-2 transition-colors flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 ${
        checked ? "border-green-400 text-green-400" : "border-gray-300 text-gray-300"
      }`}
    >
      {checked && (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" fill="currentColor" className="w-6 h-6">
          <path
            fillRule="evenodd"
